...
--b
select movie.score, movie .votes, casting.performerId
from (movie Left Outer join casting on movie.id = casting.movieId);
...
--d
select p.firstName, m.title
from casting c, movie m, performer p
where c.movieId=m.id and c.performerId=p.id;
group by p.firstName, m.title;